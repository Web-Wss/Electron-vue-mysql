  ...
  methods: {
    ...
    searchUser() {
      ...
      if (this.input == "") {
        ...
        this.$http
          .post(`/api/SearchUserServlet?value=${this.input}`)
          .then((res) => {
            console.log(res);
    ...
    sendAddUser() {
      this.$http
        .post(
          `/api/AddUserServlet?user_username=${this.form.user_username}&user_password=${this.form.user_password}&user_nickname=${this.form.user_nickname}&user_phone=${this.form.user_phone}&user_qq=${this.form.user_qq}`
        )
        .then((res) => {
          console.log(res);
          if (res.data == true) {
    ...
    DeleteUser(user) {
      ...
      this.$confirm("此操作将永久删除该用户, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          this.$http
            .post(`/api/DeleteUserServlet?user_id=${user.user_id}`)
            .then((res) => {
              console.log(res);
    ...
    sendEditUserInfoBtn() {
      this.$http
        .post(
          `/api/EditUserInfoServlet?user_id=${this.form.user_id}&user_username=${this.form.user_username}&user_nickname=${this.form.user_nickname}&user_password=${this.form.user_password}`
        )
        .then((res) => {
          console.log(res);
          if (res.data == true) {
    ...
    async change(user_id, user_status) {
      ...
      const res = await this.$http.post(
        `/api/EditUserStatusServlet?user_id=${user_id}&user_status=${user_status}`
      );
      console.log(res);
      if (res.data == true) {
        this.$message({
    ...
    // 得到用户总数
    async getUserTotal() {
      const res = await this.$http.get("/api/UserTotalServlet");
      this.user_total = res.data;
    },
    // 获取用户列表
    async getUserList() {
      const res = await this.$http.post(
        `/api/GetUserInfoServlet?currentPage=1`
      );
      console.log(res);
      for (let i = 0; i < res.data.length; i++) {
        if (res.data[i].user_status == 1) {
    ...
    handleCurrentChange(val) {
      ...
      console.log(`当前页: ${val}`);
      this.$http
        .post(`/api/GetUserInfoServlet?currentPage=${val}`)
        .then((res) => {
          console.log(res);
          for (let i = 0; i < res.data.length; i++) {
            if (res.data[i].user_status == 1) {
